...
function buildActivity(loans, repayments) {
    const events = [];
    // Each loan produces multiple timeline items for full flow tracking
    for (const l of loans){
        const amount = l.amount;
        const sublabel = `Loan · ${formatCurrency(amount)}`;
        // 1. Requested – always (every loan starts here)
        events.push({
    ...
    // Repayments – each as a separate item
...
//# sourceMappingURL=apps_stillPlay-mobile_src_app_dashboard_page_tsx_6297da49._.js.map